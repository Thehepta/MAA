...
from d810.utils import NotResolvableFatherException, get_all_possibles_values, NotDuplicableFatherException
from d810.tracker import MopTracker, duplicate_histories, SymbolicMopHistory
...
# 添加以下三行即可输出到控制台
# d810.tracker.logger.setLevel(logging.DEBUG)  # 设置日志级别
# console_handler = logging.StreamHandler()  # 创建控制台处理器
# d810.tracker.logger.addHandler(console_handler)  # 将处理器添加到logger
...
class UnflattenerFakeJump():
    DISPATCHER_CLASS = HelloOllvmDispatcherInfo
    ...
    def __init__(self):
        ...
        self.cur_maturity = hr.MMAT_ZERO
    ...
    def func(self, blk: mblock_t):
        ...
        self.last_pass_nb_patch_done = 0
        # self.auto_retrieve_all_dispatchers()
        # self.set_dispatch_block(8)
        self.set_dispatch_block(5)
        if len(self.dispatcher_list) == 0:
            return 0
        else:
            for dispatcher in self.dispatcher_list:
                print("dispatcher:", dispatcher.entry_block.blk.serial)
            self.last_pass_nb_patch_done = self.remove_flattening()

        # hello_ollvm_fla使用这个会崩溃
    ...
    def start(self):
        ...
        self.last_pass_nb_patch_done = self.remove_flattening()
        show_microcode_graph(self.mba, fn_name)

    def auto_retrieve_all_dispatchers(self):
        self.dispatcher_list = []
        for blk_idx in range(self.mba.qty):
            blk = self.mba.get_mblock(blk_idx)
            disp_info = self.DISPATCHER_CLASS(self.mba)
            if disp_info.explore(blk):
                print("dispatcher:", disp_info.entry_block.blk.serial)
                self.dispatcher_list.append(disp_info)

    def set_dispatch_block(self, block_serial):
        self.dispatcher_list = []
        ...
        for dispatcher_info in self.dispatcher_list:
            ...
            for dispatcher_father in dispatcher_father_list:
                try:
                    ...
                    total_nb_change += self.ensure_dispatcher_father_is_resolvable(dispatcher_father, dispatcher_info)
                    # if dispatcher_father.serial == 42:
                    #     return 0
            ...
            dispatcher_father_list = [self.mba.get_mblock(x) for x in dispatcher_info.entry_block.blk.predset]
            dispatcher_father_list_serial = [ x for x in dispatcher_info.entry_block.blk.predset]
            print("dispatcher_father_list_serial:",dispatcher_father_list_serial)
    ...
    def resolve_dispatcher_father(self, dispatcher_father: mblock_t, dispatcher_info) -> int:
        ...
        dispatcher_father_histories = self.get_dispatcher_father_histories(dispatcher_father,
                                                                           dispatcher_info)
        if len(dispatcher_father_histories) > 1:
            raise NotResolvableFatherException("Dispatcher {0} predecessor {1} is not resolvable"
                                               .format(dispatcher_info.entry_block.serial, dispatcher_father.serial))


            # print("tmp_mop_searched_values:", hex(tmp_mop_searched_values[0]))
    ...
    def get_dispatcher_father_histories(self, dispatcher_father: mblock_t,
                                        dispatcher_info: GenericDispatcherInfo) -> List[SymbolicMopHistory]:

        father_tracker = MopTracker(dispatcher_info.entry_block.use_before_def_list,
                                    max_nb_block=self.MOP_TRACKER_MAX_NB_BLOCK, max_path=self.MOP_TRACKER_MAX_NB_PATH)
        father_tracker.reset()
        avoid_list = [x.blk.serial for x in dispatcher_info.dispatcher_internal_blocks]
        father_histories = father_tracker.search_backward(dispatcher_father, None, avoid_list)
        return father_histories

    def check_if_histories_are_resolved(self, mop_histories: List[SymbolicMopHistory]) -> bool:
...
class blkOPt(hr.optblock_t):
    ...
    def func(self, blk):
        ...
        if isFirst:
            ...
            optimizerret = optimizer.func(blk)
            print(">>>>>>end<<<<<<")
            return optimizerret
        return 0
