...
from d810.cfg_utils import insert_nop_blk, change_2way_block_conditional_successor, insert_goto_instruction, \
    change_1way_call_block_successor, change_0way_block_successor
from ida_hexrays import mblock_t
from lucid.ui.graph import show_microcode_graph, graphviz
...
def make_insn():
    insn = hr.minsn_t(0)
    insn.opcode = hr.m_mov  # 修改这个指令的的opcode 为 add
    insn.l = hr.mop_t()
    insn.l.make_reg(13, 8)
    insn.d = hr.mop_t()
    insn.d.make_number(88, 8)
    return insn
...
def start():
    ...
    print("123456")
    block5 = mba.get_mblock(5)
    print(block5.serial)

    pred_block = mba.get_mblock(3)
    print(pred_block.serial)
    insn = make_insn()
    print("123456")

    insert_blk = insert_nop_blk(pred_block)
    print(insert_blk.serial)
    print(block5.serial)

    insert_blk.insert_into_block(insn, insert_blk.head)

    # change_0way_block_successor(insert_blk, 3)
    # insert_blk.insert_into_block(insert_insn, insert_blk.tail)
    # pred_block = mba.get_mblock(31)
    # block_to_duplicate = mba.get_mblock(4)
    # print("block_to_duplicate:",block_to_duplicate.nsucc())
    # duplicated_blk_jmp, duplicated_blk_default = duplicate_block(block_to_dup\licate)
    #
    ...
    # 将mba 的cfg显示出来
    print("123456")
    # graphviz(mba,"/home/chic/Desktop/modify.dot")
